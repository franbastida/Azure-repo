...
source $SCRIPT_ROOT/modules/xploreTestDsearch.sh
source $SCRIPT_ROOT/modules/xploreTestIndexagent.sh
...
outputHandler "## Running RUNNER: test_cs_xplore.sh - MODULE: xploreTestDsearch ##" "INFO" "$_APPLICATION" "$_OBJECT"
xploreTestDsearch rcNormal rcError "SEVERITY[$_SEVERITY]" "DOCBASES[${rcDocbasesGlobal[*]}]"
itemStateHandler "xploreTestDsearch" "$_SEVERITY" "${rcNormal[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK"
itemStateHandler "xploreTestDsearch" "$_SEVERITY" "${rcError[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK"
...
# Check state of the xPlore IndexAgent for docbases
_OBJECT="xPloreIndex"
if [ $TRACE == 1 ]; then
    outputHandler "## Running RUNNER: test_cs_xplore.sh - MODULE: xploreTestIndexagent ##" "INFO" "$_APPLICATION" "$_OBJECT"
    xploreTestIndexagent rcNormal rcError "SEVERITY[$_SEVERITY]" "DOCBASES[${rcDocbasesGlobal[*]}]";
    itemStateHandler "xploreTestIndexagent" "$_SEVERITY" "${rcNormal[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK";
    itemStateHandler "xploreTestIndexagent" "$_SEVERITY" "${rcError[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK";
