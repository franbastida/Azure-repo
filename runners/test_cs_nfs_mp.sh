...
# Load other modules
source $SCRIPT_ROOT/modules/nfsMountPointTester.sh
...
### LOCAL
_OBJECT="NFS"
outputHandler "## Running RUNNER: test_cs_nfs_mp.sh - MODULE: nfsMountPointTester ##" "INFO" "$_APPLICATION" "$_OBJECT"
nfsMountPointTester rcNormal rcError "SEVERITY[$_SEVERITY]" $TOMCAT_LIST
itemStateHandler "nfsMountPointTester" "$_SEVERITY" "${rcNormal[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK"
itemStateHandler "nfsMountPointTester" "$_SEVERITY" "${rcError[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK"
