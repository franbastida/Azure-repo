...
# Load other modules
source $SCRIPT_ROOT/modules/bravaMountPointTester.sh
...
_OBJECT="Tomcat"
outputHandler "## Running RUNNER: test_brava_mp.sh - MODULE: bravaMountPointTester ##" "INFO" "$_APPLICATION" "$_OBJECT" # Echo error message to logs/output
bravaMountPointTester rcNormal rcError "SEVERITY[$_SEVERITY]" $TOMCAT_LIST
itemStateHandler "bravaMountPointTester" "$_SEVERITY" "${rcNormal[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK"
itemStateHandler "bravaMountPointTester" "$_SEVERITY" "${rcError[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK"
