...
_OBJECT="JMS"
outputHandler "## Running RUNNER: test_cs_jms.sh - MODULE: jmsCountProcessTomcat ##" "INFO" "$_APPLICATION" "$_OBJECT"
...
if [ "${rcNormalProc[@]}" != "NORMAL[]" ]; then
    ...
    _OBJECT="JMS"
    outputHandler "## Running RUNNER: test_cs_jms.sh - MODULE: jmsTestConnection ##" "INFO" "$_APPLICATION" "$_OBJECT"
...
### Test if D2 is running properly on JMS
### GLOBAL
# Check connection to the global instance of docbase
_OBJECT="docbase"
outputHandler "## Running RUNNER: test_cs_jms.sh - MODULE: docbaseTestConnection ##" "INFO" "$_APPLICATION" "$_OBJECT"
docbaseTestConnection rcNormal rcError rcDocbasesGlobal "SEVERITY[$_SEVERITY]" "SCOPE[global]"
unset rcNormal rcError

# Run further tests if there is at least one docbase available
if [ "${rcDocbasesGlobal[@]}" != "" ]; then
    ### GLOBAL
    # Try to run D2IsJMSRunning method on the repositories
    _OBJECT="JMS"
    outputHandler "## Running RUNNER: test_cs_jms.sh - MODULE: jmsTestD2Method ##" "INFO" "$_APPLICATION" "$_OBJECT"
...
unset rcDocbasesGlobal rcNormalProc rcErrorProc
