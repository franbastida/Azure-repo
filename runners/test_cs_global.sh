...
_OBJECT="docbroker"
outputHandler "## Running RUNNER: test_cs_global.sh - MODULE: docbrokerCountProcess ##" "INFO" "$_APPLICATION" "$_OBJECT" # Echo error message to logs/output
...
if [ "${rcNormalProc[@]}" != "NORMAL[]" ]; then
    ...
    _OBJECT="docbroker"
    outputHandler "## Running RUNNER: test_cs_global.sh - MODULE: docbrokerTestConnection ##" "INFO" "$_APPLICATION" "$_OBJECT"
    ...
    if [[ ! "$rcErrorConn" == *"1489"* ]]; then
        ...
        _OBJECT="docbase"
        outputHandler "## Running RUNNER: test_cs_global.sh - MODULE: docbaseTestConnection ##" "INFO" "$_APPLICATION" "$_OBJECT"
        docbaseTestConnection rcNormal rcError rcDocbasesGlobal "SEVERITY[$_SEVERITY]" "SCOPE[global]"
        ...
        outputHandler "## Running RUNNER: test_cs_global.sh - MODULE: docbaseCountProcess ##" "INFO" "$_APPLICATION" "$_OBJECT"
        docbaseCountProcess rcNormal rcError rcDocbases "SEVERITY[$_SEVERITY]" "DOCBASES[${rcDocbasesGlobal[*]}]" 
...
unset rcDocbases rcDocbasesGlobal rcDocbasesLocal rcDocbrokers rcDocbrokerPorts rcDocbrokerPortsLocal rcNormalProc rcErrorProc rcErrorConn rcNormalConn
