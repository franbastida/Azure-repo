...
# Load other modules
source $SCRIPT_ROOT/modules/xploreCountProcess.sh
...
_OBJECT="xPlore"
outputHandler "## Running RUNNER: test_xplore.sh - MODULE: xploreCountProcess ##" "INFO" "$_APPLICATION" "$_OBJECT"
xploreCountProcess rcNormal rcError "SEVERITY[$_SEVERITY]"
itemStateHandler "xploreCountProcess" "$_SEVERITY" "${rcNormal[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK"
itemStateHandler "xploreCountProcess" "$_SEVERITY" "${rcError[@]}" "$_APPLICATION" "$_OBJECT" "$_CONFLUENCE_LINK"
